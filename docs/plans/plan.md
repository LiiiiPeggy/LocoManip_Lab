...
1. **末端执行器参考点**:框架要求 body 名 `end_effector`,URDF 里没有。
   (a) 直接指向已有的 `gripper_base_link`(省事,但它不是 TCP,位姿指令会带固定偏移);
   (b) 在 USD 里补一个 TCP frame 命名 `end_effector`(推荐,位姿语义才正确)。
2. **底盘参考系 `link0`**:WBC 的 XY 指令是相对机身 `link0` 系,而底盘叫 `base_link`。
   (a) 用 `link0_name` 参数指向 `base_link`(零改动,推荐);(b) 改名。
3. **夹爪**:与 GO2-PIPER 对齐则不进动作空间(动作 = 6 维)。若要后续做抓取再开。
4. **网格精简**:31 MB 底盘 STL 会拖慢每次启动。是否允许转换前简化(视觉抽稀、碰撞换凸包/基本体)?
   这会改变外观精度,所以问你。
5. **提交范围**(87 MB 网格 + 两份 description 副本):
   - `.gitattributes` 里 STL 走 LFS,但 pre-commit 的 `check-added-large-files`(5000 KB)
     会拒绝其中 6 个文件(最大 31 MB)。
   - 两份 description 副本(md5 一致)会漂移,必须定单一来源。
   - 建议:URDF/配置入库,网格 `.gitignore`(与现有 6 个 `*_base.usd` 的处理一致),
     并明确"以 `agx/rangerboxcr10lidar_description/` 为准,根目录那份删除或改软链"。请确认。
6. **`agx/` 的归属**:它是独立 git 仓库(origin `LiiiiPeggy/agx`,branch master),
   目前在 LocoManip_Lab 里是未跟踪目录。要不要做成 submodule,还是加 `.gitignore`?
...
1. **§4 底盘方案**:选 **B**(推荐,底盘作输入指令)还是 **C**(策略输出底盘速度)?
2. **§5 的六个决策点**(EE 参考点 / `link0` / 夹爪 / 网格精简 / 提交范围与单一来源 / agx 是否 submodule)。
3. **§2.2 阻断项**:CR10 的真实关节限位能否确认(查控制器或实测)?在你确认前我不会开始阶段 3。
4. 训练规模:GO2-PIPER 是 8192 环境 × 2500 轮 ≈ 64 分钟;本机器人自由度更多、质量更大,
   先用阶段 2 实测吞吐再定,可以吗?
