...
由"底盘速度作外部指令"改为"**RL 同时控制底盘与机械臂**"(§4)。仿真与实机的接口边界见 §6;
剩余待定项(R1–R5)都不阻塞开工,见 §9。
标 **⚠️** = 已核实的障碍或未验证项。
...
仿真里的底盘控制器要复刻实机驱动那套解算,让 `(vx, wz)` 在两边产生相同的底盘运动:
...
4. 轮速由运动学反解,四轮同速。
   **不实现 `PARALLEL`**(本期不给 `vy`),但保留分支以便日后开启。

策略每 **0.1 s** 给出一次 `(vx, wz)`(§6.5),控制器在中间的物理步**持续执行最近一次指令**。

### 4.2 ⚠️ 联合方案引出的必改项:EE 目标必须在世界系

现在 `UniformPoseWBCCommandCfg` 的 XY 是**机身(`link0`)系**、Z 是世界系(`docs/WBC_MIXED_FRAME.md`)。
**本方案下这不成立**:机身系 XY 目标不因底盘平移而改变 —— 底盘往前开,末端相对机身的位置一点没变。
于是"必须移动底盘才能到达的目标"**永远不可能出现**,RL 也学不到用底盘辅助够取,§1 目标 2 直接落空。
(机身系下只有 **yaw 旋转**能间接改变末端在机身系中的位置。)

**因此 EE 目标的 XY 必须改成世界系。** 两条路:

- **(a) 给 `UniformPoseWBCCommand` 增加世界系 XY 模式** —— 改 `mdp/pose_command_wbc.py`
  与 `command_cfg.py`,共享代码改动必须**默认关闭**以免影响其它 7 个平台;
- **(b) 另写一个 `UniformPoseWorldCommand` 命令项**,只在 `ranger_cr10` 里启用,不碰现有代码。

我倾向 **(b)**:零风险、可独立验证,也与 §3.2"WBC 路径基本只加不改"的结论一致。
**需要你确认 → §9 R5。**

> 连带影响:目标改世界系后,`position_command_error_exp`(`rewards.py:66-98`)里"XY 在 link0 系"
> 的实现也要有世界系版本(或按世界系重新实现)。**两处必须一起改**,只改一半会得到
> "命令是世界系、奖励按机身系算"的错配,训练会往错方向收敛。
...
| B7 | EE 目标的参考系 | **必须改为世界系**(否则"需要底盘才能到达的目标"不存在,§4.2)。实现方式待定 → §9 R5 |
...
5. 写 `config/ranger_cr10/` 四件套 + agents:
   ...
   - EE 目标的参考系按 §9 R5 的结论落地(世界系);`link0_name` 指向 `base_link`(§3.2)。
...
| ⚠️ **EE 目标参考系**:机身系下"必须移动底盘才能到达的目标"根本不存在 | **方案未定**(§9 R5) | 改为世界系(§4.2),**命令与奖励必须同时改** |
...
### R5. EE 目标参考系怎么改?(**最关键**,直接决定 §1 目标 2 能否成立)

§4.2 说明:机身系 XY 目标下,"必须移动底盘才能到达的目标"不存在,RL 学不到协同。
所以要改成世界系,两条实现路径:

- **(b) 新写一个 `UniformPoseWorldCommand` 命令项**,只在 `ranger_cr10` 启用 —— 我推荐这条:
  不碰共享代码,零风险,可独立验证;
- **(a) 给现有 `UniformPoseWBCCommand` 加世界系 XY 模式** —— 复用度高,但改的是共享代码,
  必须默认关闭,回归面更大。

两条路都**必须同时改奖励**(`position_command_error_exp`,§4.2 的连带影响),只改一半会错配。
**需要你定 (a) 还是 (b)**。不回复则我按 (b) 做。

---

## 10. 当前无阻断项

A 层两个阻断项都已解决(§2.2 限位已定、§5 A2 末端帧已定),**阶段 0 也已完成**(§7)。
剩余待定项都不阻塞开工:

| 项 | 影响阶段 | 默认动作 |
|---|---|---|
| **R5 EE 目标参考系** | **阶段 1(命令/观测怎么搭)与阶段 3(训练成败)** | 默认按 **(b) 新写世界系命令项**,不碰共享代码 |
| R1 末端帧 vs 控制器 TCP | 阶段 5 | 先按指尖中心做,阶段 5 前核对 |
| R3 成功判据 | 阶段 3 验收 | GO2-PIPER 量级 + 三类目标分开统计 + 冻结底盘对照组 |
| R4 是否上机 | 阶段 1/5 的深度 | 默认按"暂不上机" |

**下一步是阶段 1(接入与注册)。** R5 决定命令与观测怎么搭,最好先定。
